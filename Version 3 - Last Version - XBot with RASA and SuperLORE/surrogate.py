...
class DecTree(Surrogate):
    ...
    def learn_local_decision_tree(self, Z, Yb, weights, class_values, multi_label=False, one_vs_rest=False, cv=5,
                                  prune_tree=False):
        dt = DecisionTreeClassifier()
        if prune_tree:
            param_list = {'min_samples_split': [0.002, 0.01, 0.05, 0.1, 0.2, 5, 3, 2],
                          'min_samples_leaf': [0.001, 0.01, 0.05, 0.1, 0.2, 1, 2, 4],
                          'splitter' : ['best', 'random'],
                          'class_weight' : ['balanced', 'None'],
                          'max_depth': [None, 2, 4, 6, 10, 12, 16, 20, 30],
                          'criterion': ['entropy', 'gini'],
                          'max_features': [0.2, 1, 2, 5, 10, 'auto', 'sqrt', 'log2']
                          }
