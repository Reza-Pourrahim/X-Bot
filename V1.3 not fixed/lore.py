...
class LORE(object):
    ...
    def explain_instance(self, x, samples=1000, use_weights=True, metric=neuclidean, nbr_runs=10, upper_threshold= 4):

        if isinstance(samples, int):
            if self.verbose:
                print('generating neighborhood - %s' % self.neigh_type)
            Z = self.neighgen_fn(x, samples, nbr_runs, upper_threshold)
        else:
            Z = samples
