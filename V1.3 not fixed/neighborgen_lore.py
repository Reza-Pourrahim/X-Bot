...
class LoreNeighborhoodGenerator(object):
    ...
    def binary_sampling_search(self, z, z_label, blackbox, upper_threshold, lower_threshold=0, n=10000, n_batch=1000,
    ...
    def generate_fn(self, x, sample=1000, nbr_runs=10, upper_threshold=4):
        y_val = self.blackbox.predict(x.reshape(1, -1))[0]

        # cf: closest_counterfactual
        # bt: latest_working_threshold
        if self.neigh_type == 'gmgm':
            cf, bt = self.binary_sampling_search(x.reshape(1, -1), y_val, self.blackbox, upper_threshold,
                                                 n=10000, n_batch=1000, stopping_ratio=0.01, kind="gaussian_matched")
