...
def prepare_dataset(df, class_name, encdec):
    ...
    if encdec == 'onehot':
        ...
        df_numeric_idx = get_numeric_idx(df, class_name)
    ...
    return df, feature_names, class_values, numeric_columns, rdf, real_feature_names, features_map,\
           df_categorical_idx, df_numeric_idx
...
def get_numeric_idx(df, class_name):
    features = [c for c in df.columns if c not in [class_name]]
    numeric_columns = list(df[features]._get_numeric_data().columns)
    numeric_idx = [features.index(f) for f in numeric_columns]
    return numeric_idx
